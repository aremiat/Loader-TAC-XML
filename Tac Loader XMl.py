...
def format_date(timestamp, frequency):
    if frequency == 'M': # Met la journée au 1er du mois
        return timestamp.replace(day=1)
    elif frequency == 'Q': # Met le mois au 1er janvier avec 3 mois entre chaque observation 
        return timestamp.replace(month=1, day=1)
    elif frequency == 'A': # Met le mois au 1er janvier avec 1 an entre chaque observartion
        return timestamp.replace(month=1, day=1)
    else:
        return timestamp
...
def fetch_and_format_data(symbol, key): 
    ...
    # Formated la data en fonction des critéres voulus
    # Ainsi qu'apppliquer la fonction format_date, pour mettre la date en fonction des différentes fréqeunces au format voulu
...
fetch_and_format_data("BBDP1","M.DE.N.VPI.C.A00000.I20.A")

df = pd.DataFrame(value,frequence)
